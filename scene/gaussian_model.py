...
class GaussianModel:
    ...
    def setup_functions(self):
        ...
        self.instance_feature_activation = torch.nn.functional.normalize
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        instance_features = torch.rand(fused_point_cloud.shape[0], FEATURE_DIM, dtype=torch.float, device="cuda")
        ...
        self._instance_feature = nn.Parameter(instance_features.requires_grad_(True))
    ...
    def load_ply(self, path):
        ...
        ins_f_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("ins_f")]
        ins_f_names = sorted(ins_f_names, key = lambda x: int(x.split('_')[-1]))
        ins_fs = np.zeros((xyz.shape[0], len(ins_f_names)))
        for idx, attr_name in enumerate(ins_f_names):
            ins_fs[:, idx] = np.asarray(plydata.elements[0][attr_name])
        ...
        self._instance_feature = nn.Parameter(torch.tensor(ins_fs, dtype=torch.float, device="cuda").requires_grad_(True))
